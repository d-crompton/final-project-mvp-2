...
# Displays any saved Trusted Devices
from tkinter import *
import File_Handler
...
class TrustedWindow:
    ...
    def __init__(self):
        ...
        title_lbl = Label(frame, text="Trusted Devices").grid(row=0, padx=(10, 5), sticky="WE")
        ...
        for device in self.devices:
            ...
            name_label = Label(inner_frame, text=device.get_name(), bg=bg).grid(row=0, column=0, sticky="W")
            ...
            info_button.grid(row=0, rowspan=3, column=1, sticky="NSEW")
            row += 1
        return_btn = Button(frame, text="Return", bg="#669999", command=self.root.destroy)
        return_btn.grid(row=2, padx=10, sticky="SWE")
        self.root.mainloop()

    def info_button(self, device):
        self.root.destroy()
        window = Device_Info_Window.InfoWindow(device, "trusted_list")
